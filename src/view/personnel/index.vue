  ...
  <div class="bigscreen_lb_1">
    ...
    <div class="bigscreen_lb_1_bottom">
      ...
      <div class="bigscreen_lb_1_bottomnei">
        <Vue3SeamlessScroll :key="tiZhengJianCeListTotal" :list="tiZhengJianCeList" :step="1" :singleHeight="70" hover
          class="scrool">
          <template v-slot="{ data }">
            <div class="bigscreen_lb_1_bottom_nei" @click="handleTiZhengOpen(data)">
              <div>
                <ElTooltip :content="data?.personnelName">
                  <span>{{ data?.personnelName }}</span>
                </ElTooltip>

                <ElTooltip :content="data?.xinlv">
                  <span>{{ data?.xinlv }}</span>
                </ElTooltip>
                <ElTooltip :content="data?.xueYang">
                  <span>{{ data?.xueYang }}</span>
                </ElTooltip>
                <ElTooltip :content="data?.tiTai">
                  <span>{{ data?.tiTai }}</span>
                </ElTooltip>
                <ElTooltip :content="data?.temp">
                  <span>{{ data?.temp }}</span>
                </ElTooltip>
                <ElTooltip :content="data?.createTime">
    ...
    <div class="bigscreen_rz_bottom">
      <div class="rzDialog_bottom">
        ...
        <div class="bigscreen_rz_1_bottomnei">
          <Vue3SeamlessScroll :key="smDeviceListTableTotal" :list="smDeviceListTable" :step="1" :singleHeight="70" hover
            class="scrool">
            <template v-slot="{ data }">
              <div class="bigscreen_rz_1_bottom_nei">
                <div>
                  <ElTooltip :content="data?.deviceSn">
  ...
  <div v-show="showPersonnelDialog" class="lbDialog">
    <div class="lbDialog_top">
      ...
      <div class="lbDialog_select">
        <ElSelect v-model="healthyValue" @change="changeHealthValue" class="selectcss" size="small">
...
const { tiZhengJianCeList, tiZhengJianCeListTotal, tiZhengPageInfo, getTiZhengJianList, tiZhengJianDetailVis,
...
const { smDeviceListTable, smDeviceListTableTotal } = useZaiXianShengMingDevice()
...
const indicatorStatisticsList = async () => {
  indicatorStatistics({
...
const indicatorStatisticsListTimer = useIntervalFn(() => {
  indicatorStatisticsListTimer.pause();
  indicatorStatisticsList().finally(() => {
    indicatorStatisticsListTimer.resume();
  });
}, 5000);
...
const accesscontrolFun = async () => {
  ...
  if (data.data.total != accesscontrollistTotal.value) {
    accesscontrollistTotal.value = data.data.total;
    accesscontrollist.value = list.map((item: any, index: number) => {
...
const healthylistTimer = useIntervalFn(() => {
  ...
  healthylistFun().finally(() => {
    healthylistTimer.resume();
  });
}, 5000);
...
onMounted(() => {
  if (showPersonnelDialogRef.value) {
